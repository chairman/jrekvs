...
import io.jrekvs.util.BytesUtil;
import io.jrekvs.util.ItemChunkUtil;
import io.jrekvs.util.ItemUtil;
import io.jrekvs.util.UnSafeUtil;
import org.apache.commons.lang.StringUtils;

import io.jrekvs.enums.DELTA_RESULT_TYPE;
import io.jrekvs.enums.ItemFlags;
import io.jrekvs.enums.Store_item_type;
import io.jrekvs.net.command.Command;
...
//		UnSafeUtil.unsafe.freeMemory;
	...
	public DELTA_RESULT_TYPE do_add_delta(Connection conn,String key,int nkey,boolean incr,long delta,byte[] buf,long hv) throws IOException{
		...
		byte[] valueByte = value.toString().getBytes(JrekvsGlobalConfig.defaultCahrset);
		...
		if((res +2) <= ItemUtil.getNbytes(it) && ItemUtil.getRefCount(it)==2){
			...
			UnSafeUtil.setBytes(ItemUtil.ITEM_data(it),
								(value.toString() + pad).getBytes(JrekvsGlobalConfig.defaultCahrset),
								0, 
			...
			ItemUtil.setValue(new_it, (value.toString() + "\r\n").getBytes(JrekvsGlobalConfig.defaultCahrset));
