...
import io.jrekvs.hash.impl.algorithm.Murmur3_64hash;
...
import io.jrekvs.context.JcacheContext;
import io.jrekvs.context.StatsState;
...
import io.jrekvs.hash.impl.AssocImpl;
import io.jrekvs.hash.impl.algorithm.Jenkins_hash;
import io.jrekvs.hash.impl.algorithm.Murmur3_32hash;
import io.jrekvs.memory.DefaultSlabsImpl;
import io.jrekvs.memory.Slabs;
import io.jrekvs.net.strategy.ReactorSelectEnum;
import io.jrekvs.net.strategy.ReactorStrategy;
import io.jrekvs.net.strategy.RoundRobinStrategy;
 ...
 *@author Tommy.z
 */
public class JrekvsMain
{
	private static final Logger logger = LoggerFactory.getLogger(JrekvsMain.class);
...
//		stats_prefix_init();  //TODO
    ...
    private static void startJcacheServer() throws IOException {
    	...
    	int poolsize = ConfigLoader.getIntProperty("reactor.pool.size", JrekvsGlobalConfig.defaulePoolSize);
    	String bindIp = ConfigLoader.getStringProperty("reactor.pool.bindIp", JrekvsGlobalConfig.defaultPoolBindIp);
    	String reaStrategy = ConfigLoader.getStringProperty("reactor.pool.selectStrategy", JrekvsGlobalConfig.defaultReactorSelectStrategy);
    	int backlog = ConfigLoader.getIntProperty("acceptor.max_connect_num", JrekvsGlobalConfig.defaultMaxAcceptNum);
    	NIOReactorPool reactorPool = new NIOReactorPool(poolsize,reactorStrategy.get(ReactorSelectEnum.valueOf(reaStrategy)));
    	
    	TCPNIOAcceptor m_acceptor=new TCPNIOAcceptor(bindIp,port, reactorPool,backlog, TCPNIOAcceptor.AcceptModel.MEMCACHE);//memcache 监听
    	TCPNIOAcceptor r_acceptor=new TCPNIOAcceptor(bindIp,redis_port, reactorPool,backlog, TCPNIOAcceptor.AcceptModel.REDIS);//redis 监听
