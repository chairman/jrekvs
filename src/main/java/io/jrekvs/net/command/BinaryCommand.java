...
import io.jrekvs.net.conn.handler.BinaryRequestHeader;
...
import io.jrekvs.enums.protocol.binary.ProtocolDatatypes;
import io.jrekvs.enums.protocol.binary.ProtocolMagic;
...
public interface BinaryCommand extends Command{
	...
	public default void process_bin_update(Connection conn) throws IOException{
		...
		if(value.remaining()> JrekvsGlobalConfig.VALUE_MAX_LENGTH){
...
//	        pthread_mutex_unlock(&c->thread->stats.mutex);
			...
			if(CommandType.touch.equals(conn.getCurCommand())){
				...
				skey = keystr.getBytes(JrekvsGlobalConfig.defaultCahrset);
			}
